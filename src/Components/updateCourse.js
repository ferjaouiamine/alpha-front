...
const UpdateCourse = (props) => {
  axios.create({
    baseURL: `https://37.59.204.215:3001/api/`,
  });
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      const course = await axios.put(
        `https://37.59.204.215:3001/api/course/${location.state.rowData._id}`,
        newCourse
      );
      setCourse(course);
...
*/
  ...
  useEffect(() => {
    const pdfFiles = () => {
      location.state.rowData.pdfUrl.map((file) => {
        defaultfiles.push({
          name: file,
          url: `https://37.59.204.215:3001/api/${file}`,
        });
        return defaultfiles;
      });
    };
  ...
  const addFiles = {
    name: "file",
    multiple: true,
    action: "https://37.59.204.215:3001/api/upload/uploadFile",
    beforeUpload: (file) => {
