...
const CoursePage = () => {
                  ...
                  {video.map((v, i) => (
                    <Panel header={v} key={i + 1}>
                      <video
                        ...
                        width="100%"
                        height="100%"
                        src={`http://https://localhost:3001/uploads/${v}`}
                      ></video>
                    </Panel>
                  ))}
                </Collapse>
                    ...
                    return (
                      <Panel header={p} key={i + 1}>
                        <iframe
                          ...
                          width="100%"
                          height="600"
                          src={`http://https://localhost:3001/uploads/${p}#toolbar=0`}
                        ></iframe>
                      </Panel>
                    );
                  })}
  ...
  useEffect(() => {
    const getCourses = async () => {
      try {
        const coursesRes = Axios.get(`http://https://localhost:3001/api/course/${id}`);
